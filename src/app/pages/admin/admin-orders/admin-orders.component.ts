...
import { AdminService, UpdateOrderStatusRequest } from '../../../core/services/admin.service';
...
export class AdminOrdersComponent implements OnInit {
    ...
    this.adminService.getAllOrders().subscribe({
      next: (response) => {
        if (response.success) {
          ...
          this.snackBar.open(response.message || 'Error al cargar órdenes', 'Cerrar', {
            duration: 3000
          });
        }
        this.loading = false;
  ...
  updateOrderStatus(order: Order, newStatus: string): void {
    ...
    this.adminService.updateOrderStatus(request).subscribe({
      next: (response) => {
        if (response.success) {
          this.snackBar.open('Estado de orden actualizado exitosamente', 'Cerrar', {
            duration: 3000
          });
          this.loadOrders();
        } else {
          this.snackBar.open(response.message || 'Error al actualizar estado', 'Cerrar', {
            duration: 3000
          });
        }
      },
